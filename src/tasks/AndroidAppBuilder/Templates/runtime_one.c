...
#define LOG_INFO(fmt, ...) __android_log_print(ANDROID_LOG_DEBUG, "DOTNET-RUNTIME-ONE", fmt, ##__VA_ARGS__)
#define LOG_ERROR(fmt, ...) __android_log_print(ANDROID_LOG_ERROR, "DOTNET-RUNTIME-ONE", fmt, ##__VA_ARGS__)
...
unhandled_exception_handler (MonoObject *exc, void *user_data)
{
    ...
    LOG_ERROR("UNHANDLED EXCEPTION IN RUNTIME-ONE");
...
void
runtime_one_init (const char* bundle_path, int local_date_time_offset)
{
    ...
    appctx_values[3] = "DOTNET-RUNTIME-ONE";
...
int
runtime_one_exec (const char* executable, int managed_argc, char* managed_argv[])
{

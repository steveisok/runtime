...
#define LOG_INFO(fmt, ...) __android_log_print(ANDROID_LOG_DEBUG, "DOTNET-RUNTIME-TWO", fmt, ##__VA_ARGS__)
#define LOG_ERROR(fmt, ...) __android_log_print(ANDROID_LOG_ERROR, "DOTNET-RUNTIME-TWO", fmt, ##__VA_ARGS__)
...
mono_droid_load_assembly (const char *name, const char *culture)
{
    ...
    if (culture && strcmp (culture, ""))
        res = snprintf (path, sizeof (path) - 1, "%s/%s/%s", dummy_path, culture, filename);
    else
        res = snprintf (path, sizeof (path) - 1, "%s/%s", dummy_path, filename);
    assert (res > 0);
...
load_aot_data (MonoAssembly *assembly, int size, void *user_data, void **out_handle)
{
    *out_handle = NULL;
    ...
    res = snprintf (path, sizeof (path) - 1, "%s/%s.aotdata", dummy_path, aname);
    assert (res > 0);
...
unhandled_exception_handler (MonoObject *exc, void *user_data)
{
    ...
    LOG_ERROR("UNHANDLED EXCEPTION IN RUNTIME-TWO");
...
void
runtime_two_init (const char* bundle_path, int local_date_time_offset)
{
    dummy_path = bundle_path;
    ...
    appctx_keys[3] = "EMBEDDED_RUNTIME_ID";
    ...
    appctx_values[3] = "DOTNET-RUNTIME-TWO";
    ...
    domain = mono_jit_init_version ("dotnet.runtimeone", "mobile");
    assert (domain);
}

int
runtime_two_exec (const char* executable, int managed_argc, char* managed_argv[])
{
